...
export default function CrmTab({ store }: { store: CaminhoLivreStore }) {
  ...
  return (
    <div className="px-5.5 pb-6 pt-5">
      ...
      <div className="mb-4 text-xs leading-relaxed text-muted">{tr.crmSub}</div>
      ...
      <div className="mb-4.5 grid grid-cols-2 gap-2">
        {crmStats.map((cs) => (
          <div key={cs.key} className="rounded-[14px] border border-[rgba(var(--border-rgb),0.08)] bg-card px-3 py-2.5">
            ...
            <div className="mt-0.5 text-[10.5px] text-muted">{cs.label}</div>
          </div>
        ))}
      </div>

      {state.debts.length === 0 && <div className="mb-4 text-xs leading-relaxed text-muted">{tr.crmEmptyState}</div>}
          ...
          <div key={d.id} className="mb-3 rounded-2xl border border-[rgba(var(--border-rgb),0.08)] bg-card p-3.5">
            <div className="mb-2.5 flex items-center gap-2">
              <input
                ...
                className="min-w-0 flex-1 rounded-[9px] border border-[rgba(var(--border-rgb),0.1)] bg-bg px-2.5 py-2.5 text-[13px] font-semibold text-text outline-none"
              />
              <div
                onClick={() => actions.cycleCrmStatus(d.id)}
                className="cursor-pointer whitespace-nowrap rounded-full px-2.5 py-1.5 text-[10.5px] font-bold"
              ...
              <div
                ...
                className="flex h-[30px] w-[30px] shrink-0 cursor-pointer items-center justify-center rounded-[9px] border border-[rgba(var(--border-rgb),0.1)] bg-bg text-[13px] text-muted"
              >
                ✕
              </div>
            </div>
            <div className="mb-2.5 font-display text-[15px] font-bold text-orange">{money(d.balance)}</div>
            <div className="mb-2 grid grid-cols-2 gap-1.5">
              <label className="block">
                <span className="mb-1 block text-[9.5px] text-muted">{tr.crmPhoneLabel}</span>
                <input
                  value={d.phone}
              ...
              <label className="block">
                <span className="mb-1 block text-[9.5px] text-muted">{tr.crmNextFollowUpLabel}</span>
                <input
            ...
            <label className="mb-2.5 block">
              <span className="mb-1 block text-[9.5px] text-muted">{tr.crmNotesLabel}</span>
              <input
                value={d.notes}
            ...
            <div className="flex gap-2">
              {hasPhone && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex-1 rounded-full border border-[rgba(var(--accent-rgb),0.3)] py-2 text-center text-[11.5px] font-semibold text-accent no-underline"
                >
                  WhatsApp
                </a>
              )}
              <div
                ...
                className="flex-1 cursor-pointer rounded-full border border-[rgba(var(--gold-rgb),0.35)] py-2 text-center text-[11.5px] font-semibold text-gold"
              >
      ...
      <div
        ...
        className="cursor-pointer rounded-xl border border-dashed border-[rgba(var(--border-rgb),0.2)] px-3 py-3 text-center text-[13px] font-semibold text-accent"
      >
        {tr.crmAddCreditor}
      </div>
    </div>
