    ...
    <div className="px-5.5 pb-6 pt-5">
      ...
      <div className="mb-4 flex gap-2">
        <div onClick={() => actions.setStrategy("snowball")} className={strategyBtn(state.strategy === "snowball")}>
          <div className="text-[14.5px] font-bold">{tr.snowball}</div>
          <div className="mt-0.5 text-[12.5px] leading-snug">{tr.snowballDesc}</div>
        </div>
        <div onClick={() => actions.setStrategy("avalanche")} className={strategyBtn(state.strategy === "avalanche")}>
          <div className="text-[14.5px] font-bold">{tr.avalanche}</div>
          <div className="mt-0.5 text-[12.5px] leading-snug">{tr.avalancheDesc}</div>
        </div>
      </div>

      {cashWarningVisible && (
        <div className="mb-3.5 rounded-xl border border-[rgba(var(--orange-rgb),0.3)] bg-[rgba(var(--orange-rgb),0.1)] px-3.5 py-2.5 text-[13.5px] text-orange">
          {tr.cashWarning}
        </div>
      )}
      ...
      {state.debts.map((d) => (
        <div key={d.id} className="relative mb-2.5 rounded-2xl border border-[rgba(var(--border-rgb),0.08)] bg-card p-3.5">
          {stats.focusDebt?.id === d.id && (
            <div className="absolute -top-2 right-3 rounded-full bg-accent px-2 py-0.5 text-[11.5px] font-bold text-[#0D1311]">
              {tr.focusDebtTitle}
            </div>
          )}
          <div className="mb-2 flex gap-2">
            <input
              ...
              className="min-w-0 flex-1 rounded-[9px] border border-[rgba(var(--border-rgb),0.1)] bg-bg px-2.5 py-2.5 text-[14.5px] text-text outline-none"
            />
            <div
              onClick={() => actions.removeDebt(d.id)}
              className="flex h-[34px] w-[34px] shrink-0 cursor-pointer items-center justify-center rounded-[9px] border border-[rgba(var(--border-rgb),0.1)] bg-bg text-base text-muted"
            >
              ✕
            </div>
          </div>
      ...
      <div
        ...
        className="mb-4 cursor-pointer rounded-xl border border-dashed border-[rgba(var(--border-rgb),0.2)] px-3 py-3 text-center text-[14.5px] font-semibold text-accent"
      >
        {tr.addDebt}
      </div>
      ...
      {hasFocusDebt && stats.focusDebt && (
        <div className="rounded-2xl border border-[rgba(var(--border-rgb),0.08)] bg-card p-4">
          <div className="text-[12px] font-semibold uppercase tracking-wide text-muted">{tr.focusDebtTitle}</div>
          <div className="mt-1 font-display text-lg font-bold text-text">{stats.focusDebt.name}</div>
          <div className="mt-3 flex flex-wrap gap-6.5">
            <div>
              <div className="text-[11.5px] uppercase text-muted">{tr.freeUntilDebtFree}</div>
              <div className="mt-0.5 font-display text-[18px] font-bold text-accent">{monthsLabel(stats.sim.months, tr)}</div>
            </div>
            <div>
              <div className="text-[11.5px] uppercase text-muted">{tr.interestPaid}</div>
              <div className="mt-0.5 font-display text-[18px] font-bold text-orange">{money(stats.sim.totalInterest)}</div>
            </div>
          </div>
          {showOtherNote && (
      ...
      {stallWarningVisible && (
        <div className="mt-3 rounded-xl border border-[rgba(var(--orange-rgb),0.3)] bg-[rgba(var(--orange-rgb),0.1)] px-3.5 py-2.5 text-[13.5px] text-orange">
          {tr.stallWarning}
        </div>
      )}
    </div>
