  ...
  return (
    <div className="flex h-full flex-col">
      <div className="px-6 pt-8">
        ...
        <p className="my-1.5 mb-5 text-[14.5px] text-muted">{tr.activationSub}</p>
      </div>
      <div className="flex-1 overflow-auto px-6">
        <div className="mb-4.5 rounded-xl border border-[rgba(var(--gold-rgb),0.32)] bg-[rgba(var(--gold-rgb),0.1)] px-3.5 py-3.5">
          <div className="mb-1 text-[13px] font-semibold text-gold">{tr.codeSentDemo}</div>
        ...
        {state.activationError && (
          <div className="mb-2 text-center text-sm text-orange">{tr.wrongCode}</div>
        )}
        <div className="mt-2.5 text-center">
          <span onClick={actions.resendCode} className="cursor-pointer text-[14.5px] font-semibold text-accent">
            {tr.resendCode}
          </span>
        </div>
      </div>
