  ...
  return (
    <div className="flex h-full flex-col">
        ...
        {LANGUAGE_LIST.map((l) => {
          ...
          return (
            <div
              ...
              className={`cursor-pointer rounded-full px-2.5 py-1.5 text-[13px] font-bold ${
      ...
      <div className="flex flex-col items-center px-6 pt-4">
        ...
        <div className="mt-2.5 font-display text-lg font-bold text-text">Caminho Livre</div>
        <div className="mt-1 text-center text-[13.5px] text-muted">{tr.tagline}</div>
      </div>
      ...
      <div className="flex-1 overflow-auto px-6 pt-5">
        ...
        <p className="my-1.5 mb-5 text-[14.5px] leading-relaxed text-muted">{tr.loginSub}</p>

        <label className="mb-3 block">
          <span className="mb-1.5 block text-sm font-semibold text-muted">{tr.nameLabel}</span>
          <TextField value={state.name} onChange={(e) => actions.setName(e.target.value)} />
        </label>
        <label className="mb-3 block">
          <span className="mb-1.5 block text-sm font-semibold text-muted">{tr.phoneLabel}</span>
          <TextField value={state.phone} onChange={(e) => actions.setPhone(e.target.value)} placeholder="+244 9XX XXX XXX" />
        </label>
        <label className="mb-2 block">
          <span className="mb-1.5 block text-sm font-semibold text-muted">{tr.passwordLabel}</span>
        ...
        <div className="mb-5 flex justify-end">
          <span className="cursor-pointer text-[14.5px] font-semibold text-accent">{tr.forgotPassword}</span>
        </div>
        ...
        <div className="my-5 flex items-center gap-2.5">
          ...
          <div className="text-[13.5px] text-muted">{tr.orWord}</div>
          <div className="h-px flex-1 bg-[rgba(var(--border-rgb),0.12)]" />
        </div>

        <div
          onClick={actions.goSignup}
          className="cursor-pointer rounded-xl border border-[rgba(var(--border-rgb),0.18)] px-3.5 py-[13px] text-center text-base font-semibold text-text"
        >
          {tr.createAccount}
        </div>
      </div>

      <div className="px-6 pb-5 pt-3.5 text-center text-[12.5px] text-muted">{tr.dataPrivacy}</div>
    </div>
  );
}
