  ...
  return (
    <div id="cl-print-report" className="absolute left-[-99999px] top-0 w-[800px] bg-white p-12 font-sans text-[#16201C]">
      ...
      <div className="my-7 flex gap-10">
        <div>
          <div className="text-[11px] uppercase text-[#666]">{tr.totalDebt}</div>
          <div className="mt-1 text-[22px] font-bold">{money(stats.totalDebt)}</div>
        </div>
        <div>
          <div className="text-[11px] uppercase text-[#666]">{tr.freeThisMonth}</div>
          <div className="mt-1 text-[22px] font-bold">{money(stats.freeCash)}</div>
        </div>
        <div>
          <div className="text-[11px] uppercase text-[#666]">{tr.strategyLabel}</div>
          <div className="mt-1 text-[22px] font-bold">{strategyDisplayLabel}</div>
        </div>
        <div>
          <div className="text-[11px] uppercase text-[#666]">{tr.freeUntilDebtFree}</div>
          <div className="mt-1 text-[22px] font-bold">{monthsLabel(stats.sim.months, tr)}</div>
        </div>
      </div>
      <div className="mb-2.5 text-base font-bold">{tr.debtsTitle}</div>
      {state.debts.map((d) => (
        <div key={d.id} className="flex justify-between border-b border-[#e5e5e5] py-2.5 text-[13px]">
          <div>{d.name}</div>
          <div>{money(d.balance)}</div>
        </div>
      ))}
      <div className="mb-2.5 mt-6 text-base font-bold">{tr.rulesLabel}</div>
